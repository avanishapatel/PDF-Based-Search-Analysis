...
st.set_page_config(
    page_title="PDF Based Search & Analysis",
    page_icon="📚",
    layout="wide"
)
...
def process_pdf_data(uploaded_file):
    ...
    pdf_processor = get_pdf_processor()
    num_chunks, total_pages, page_images = pdf_processor.process_pdf(uploaded_file)
    return {
                    ...
                    if search_results:
                        ...
                        if summary == "Sorry, no relevant content found for your query.":
                            ...
                            for doc in source_documents:
                                ...
                                with col2:
                                    st.markdown(f'<span class="document-name">{doc["pdf_name"]}</span>', 
                                                unsafe_allow_html=True)
